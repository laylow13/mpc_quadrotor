...
int main() {
    double ts = 0.05;
    int N = 30;
    LMPC_Full::problem_params_t problem_params{};
    problem_params.ts = ts;
    problem_params.N = N;
    LMPC_Full::model_params_t model_params{};
    // simulation settings
    LMPC_Full controller(problem_params, model_params);
    ...
    MX open_traj = res["x"](Slice(4 * N, 4 * N + 12 * (N + 1)));
    open_traj = reshape(open_traj, 12, N + 1);
