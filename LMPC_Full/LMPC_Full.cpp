...
#include "LMPC_Full.hpp"

LMPC_Full::LMPC_Full(LMPC_Full::problem_params_t problem_params_, LMPC_Full::model_params_t model_params_) : problem_params(
        problem_params_), model_params(model_params_) {
    init_mpc_problem();
    init_solver();
}

DMDict LMPC_Full::compute(const DM &current_state_, const DM &traj_) {
...
void LMPC_Full::init_solver(std::string solver_type, std::map<std::string, GenericType> solver_opts) {
    mpc_solver = nlpsol("solver", solver_type, nlp, solver_opts);

}

void LMPC_Full::get_mpc_problem(MXDict &problem_, std::vector<double> &lbg_, std::vector<double> &ubg_) const {
    problem_ = nlp;
    lbg_ = lbg;
    ubg_ = ubg;
}

void LMPC_Full::set_mpc_problem(const MXDict &problem_, const std::vector<double> &lbg_, const std::vector<double> &ubg_) {
    nlp = problem_;
    lbg = lbg_;
    ubg = ubg_;
}

Function LMPC_Full::linearized_dynamics(LMPC_Full::model_params_t params_) {
    double g = params_.g;
    double m = params_.m;
    ...
    auto thrust = f * vertcat(sin(theta), -sin(phi), 1);
...
DM LMPC_Full::compute_mixing(double _cf, double _ctf, double _l) {
...
void LMPC_Full::init_mpc_problem() {
    Function dynamics = linearized_dynamics(model_params);
