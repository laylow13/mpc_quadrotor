...
Function NMPC_Euler::euler_dynamics(model_params_t params_) {
    double g = params_.g;
    double m = params_.m;
...
NMPC_Euler::NMPC_Euler(problem_params_t problem_params_, model_params_t model_params_) :
        problem_params(
                problem_params_),
        model_params(
                model_params_) {
...
void NMPC_Euler::init_mpc_problem() {
...
DMDict NMPC_Euler::compute(const DM &current_state_, const DM &traj_) {
...
void NMPC_Euler::init_solver(std::string solver_type, std::map<std::string, GenericType> solver_opts) {
    mpc_solver = nlpsol("solver", solver_type, nlp, solver_opts);

}

void NMPC_Euler::get_mpc_problem(MXDict &problem_, std::vector<double> &lbg_, std::vector<double> &ubg_) const {
    problem_ = nlp;
    lbg_ = lbg;
    ubg_ = ubg;
}
...
void
NMPC_Euler::set_mpc_problem(const MXDict &problem_, const std::vector<double> &lbg_, const std::vector<double> &ubg_) {
    nlp = problem_;
    lbg = lbg_;
    ubg = ubg_;
}

DM NMPC_Euler::compute_mixing(double _cf, double _ctf, double _l) {
